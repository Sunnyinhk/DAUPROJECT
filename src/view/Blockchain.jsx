...
const isWalletConnected = async () =>{
    try{
        if(!ethereum) return alert('please install metasmask')
...
const getEthereumContract=async()=>{
    const connectedAccount = getGloablState('connectedAccount')
...
const listVoters = async(id) =>{
    try{
        const contract=await getEtheruemContract()
...
const payoutBeneficiary = async(id)=>{
    try{
        const contract=await getEthereumContract()
        const account = getGlobalState('connectedAccount')
        await contract.methods.payBeneficiary(id).send({frpm:account})
        window.location.reload()
